 ...
 */
 ...
 Services['ContentDirectory'] = require('./services/ContentDirectory')
 ...
 * @param {Number} port
 */
export const Sonos = function Sonos (host, port, options) {
  this.host = host
  this.port = port || 1400
...
Sonos.prototype.request = function (endpoint, action, body, responseTag, callback) {
  fetch('http://' + this.host + ':' + this.port + endpoint, {
    method: 'POST',
    headers: {
      'SOAPAction': action,
    ...
    ;(new xml2js.Parser()).parseString(response, function (err, json) {
...
Sonos.prototype.currentTrack = function (callback) {
  ...
  return this.request(this.options.endpoints.transport, action, body, responseTag, function (err, data) {
    ...
    var relTime = (parseInt(data[0].RelTime[0].split(':')[0], 10) * 60 * 60) +
      (parseInt(data[0].RelTime[0].split(':')[1], 10) * 60) +
      parseInt(data[0].RelTime[0].split(':')[2], 10)
    if ((metadata) && (metadata[0].length > 0) && metadata[0] !== 'NOT_IMPLEMENTED') {
      ...
      var track = { position: position, duration: duration, relTime: relTime }
 ...
 */
Sonos.prototype.queueSpotify = async function (track_id, callback) {
  var rand = '00030020'
  ...
  // este metodo no es el que fran creo, es una pequenia modificacion del de abajo
  //
  this.queue({
    uri: uri,
    metadata: meta
...
Sonos.prototype.addListSpotify = function (playlist_id, user_id, callback) {
  // var rand = Math.floor(Math.random() * (99999999 - 10000000 + 1) + 10000000)

  var uri = `x-rincon-cpcontainer:0006206cspotify%3auser%3a${user_id}%3aplaylist%3a${playlist_id}`

  var meta = `<DIDL-Lite xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:upnp="urn:schemas-upnp-org:metadata-1-0/upnp/" xmlns:r="urn:schemas-rinconnetworks-com:metadata-1-0/" xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/"><item id="0006206cspotify%3auser%3a${user_id}%3aplaylist%3a${playlist_id}" restricted="true"><upnp:class>object.container.playlistContainer</upnp:class><desc id="cdudn" nameSpace="urn:schemas-rinconnetworks-com:metadata-1-0/">SA_RINCON2311_X_#Svc2311-90d90ab0-Token</desc></item></DIDL-Lite>`
...
Sonos.prototype.deviceDescription = function (callback) {
  fetch('http://' + this.host + ':' + this.port + '/xml/device_description.xml')
  .then(response => response.text()).then(response => {
    ;(new xml2js.Parser()).parseString(response, (err, json) => {
      if (err) return callback(err)
      var output = {}
...
Sonos.prototype.getTopology = function (callback) {
  ...
  .then(response => response.text()).then(response => {
    ;(xml2js).parseString(response, function (err, topology) {
      if (err) return callback(err)
...
export class Search extends EventEmitter {
  constructor (options) {
    super()
    let self = this
    let client = new Client() // This would use ssdp for discovering purposes
    client.search('urn:schemas-upnp-org:device:ZonePlayer:1')

    client.addListener('response', function (headers, statusCode, rinfo) {
      var model = headers.SERVER || null
      var sonos = new Sonos(rinfo.address)
 ...
 */
export const search = function (options, listener) {
  ...
  const client = new Search(options)
  if (typeof listener === 'function') {
    client.addListener('DeviceAvailable', listener)
  }
  return client
}
